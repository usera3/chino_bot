...
async def handle_message(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        if image_urls:
            # 导入图像识别工具
            from tools.vision_tool import ImageAnalysisTool
            vision_tool = ImageAnalysisTool()
            
            # 分析每张图片
            image_descriptions = []
            for i, img_url in enumerate(image_urls, 1):
                print(f"🖼️ 正在分析第 {i} 张图片...")
                description = vision_tool._run(img_url, user_input if user_input != "请分析这张图片" else "请详细描述这张图片的内容")
                image_descriptions.append(f"[图片{i}] {description}")
            
            # 将图片描述添加到用户输入
            combined_input = f"{enhanced_user_input}\n\n" + "\n\n".join(image_descriptions)
            
            # 使用 Butler 处理（包含图片描述）- 使用异步方法
            response = await butler.aprocess(combined_input, user_id=user_id)
        else:
            # 没有图片，直接处理文本 - 使用异步方法
            response = await butler.aprocess(enhanced_user_input, user_id=user_id)
