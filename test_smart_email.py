...
async def test_smart_email():
    ...
    set_current_context(user_id="1446437177", group_id=878812866)
    
    # 测试场景：用户说"给我发个邮件"
    print("\n📝 测试场景：用户说「给我发个邮件」")
    print("-" * 60)
    
    user_input = "[系统提示：发送此消息的用户是测试用户(QQ:1446437177)，当前在群878812866中。你的 QQ 号是 2509109290，用户的 QQ 号是 1446437177]\n\n给我发个邮件"
    
    response = await butler.aprocess(user_input, user_id="1446437177")
    ...
    # 2. 根据 QQ 号推理邮箱地址（1446437177@qq.com）
    # 3. 调用 send_email 工具发送邮件
    # 4. 不应该询问用户邮箱是什么
    
    print("\n✅ 预期行为:")
    print("1. 调用 get_user_info 工具获取 QQ 号")
    print("2. 推理邮箱地址为 1446437177@qq.com")
