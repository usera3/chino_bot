...
- **发送附件**：用户说"把这个文件发到邮箱" → 调用时传入 `attachment_path` 参数
  ...
  * 附件路径示例：`/Users/mozi100/Downloads/test_send_document.docx`
        ...
        try:
            ...
            if self.use_dual_memory and self.dual_store:
                relevant_context = self.dual_store.get_relevant_context(
                    query=user_input,
                    user_id=user_id,
                    k_conversations=2,  # 向量检索对话数量（优化：3→2，减少33%）
                    k_knowledge=1,      # 知识检索数量（优化：2→1，减少50%）
                    k_recent=10         # 最近对话数量（优化：20→10，减少50%）
                )
            ...
            if self.use_dual_memory and self.dual_store:
                ...
                # 【性能优化】知识提取改为异步（不阻塞响应）
                # 注意：这里使用同步方法，所以暂时保持同步提取
                # 在异步方法 aprocess 中会使用真正的异步提取
                    ...
                    async def extract_and_save_knowledge():
                        try:
                            ...
                            if knowledge:
                                ...
                                if self.verbose:
                                    print(f"💾 [后台] 已保存知识到长期记忆")
                        except Exception as e:
                            if self.verbose:
                                print(f"⚠️ [后台] 知识提取失败: {e}")
                    
                    # 启动后台任务（不等待）
                    asyncio.create_task(extract_and_save_knowledge())
                    
                    if self.verbose:
                        print(f"💾 已保存对话到长期记忆（知识提取在后台进行）")
                else:
                    if self.verbose:
                        print(f"💾 已保存到长期记忆（对话）")
