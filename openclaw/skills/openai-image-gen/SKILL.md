...
description: Generate images via OpenAI Responses native image_generation (gpt-5.5) or Images API; can print MEDIA lines for chat delivery.
...
Generate images with OpenAI. Default mode uses `gpt-5.5` through the Responses API native `image_generation` tool; legacy `gpt-image-*`/DALL-E models still use the Images API.
...
```bash
python3 {baseDir}/scripts/gen.py --model gpt-5.5 --media-lines
