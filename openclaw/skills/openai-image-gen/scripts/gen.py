...
def read_codex_openai_api_key() -> str:
...
def request_responses_image(
    ...
    endpoint = (base_url.strip() or os.environ.get("OPENAI_BASE_URL", "").strip() or "https://api.openai.com").rstrip("/")
    ...
    try:
        ...
        payload = e.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"OpenAI Responses image generation failed ({e.code}): {payload}") from e


def extract_responses_image_b64(res: dict) -> str:
    for item in res.get("output", []) or []:
        ...
        for part in item.get("content", []) or []:
            if part.get("type") in {"output_image", "image"}:
                ...
                if part.get("result"):
                    return part["result"]
    raise RuntimeError(f"Unexpected Responses image output: {json.dumps(res)[:400]}")
...
def main() -> int:
    ...
    ap.add_argument("--model", default="gpt-5.5", help="Image model id. gpt-* uses Responses API image_generation; gpt-image*/dall-e* uses Images API.")
    ap.add_argument("--base-url", default="", help="API base URL for Responses mode (default OPENAI_BASE_URL or https://api.openai.com).")
    ap.add_argument("--media-lines", action="store_true", help="Print MEDIA: <path> lines for OpenClaw/QQ delivery.")
    ...
    api_key = (os.environ.get("OPENAI_API_KEY") or read_codex_openai_api_key()).strip()
    if not api_key:
    ...
    count = args.count
    responses_mode = args.model.startswith("gpt-") and not args.model.startswith("gpt-image")
    ...
    for idx, prompt in enumerate(prompts, start=1):
        ...
        if responses_mode:
            res = request_responses_image(
                api_key,
                prompt,
                args.model,
                size,
                quality,
                normalized_output_format or "png",
                args.base_url,
            )
            filepath.write_bytes(base64.b64decode(extract_responses_image_b64(res)))
        else:
            res = request_images(
                api_key,
                prompt,
        ...
        if args.media_lines:
            print(f"MEDIA: {filepath.resolve().as_posix()}")
