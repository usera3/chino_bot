  ...
  const handleWebSocketMessage = useCallback((message: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'set-model-and-conf':
        ...
        if (message.model_info?.fallbackImageUrl && !message.model_info.fallbackImageUrl.startsWith("http")) {
          // eslint-disable-next-line no-param-reassign
          message.model_info.fallbackImageUrl = baseUrl + message.model_info.fallbackImageUrl;
        }
      ...
      case 'audio':
        if (aiState === 'interrupted' || aiState === 'listening') {
          ...
          if (typeof window !== 'undefined') {
            const nextMotion =
              message.actions?.motions?.[0]
                ? JSON.stringify(message.actions.motions[0])
                : '';
            const nextExpression =
              Array.isArray(message.actions?.expressions) && message.actions.expressions.length > 0
                ? String(message.actions.expressions[0])
                : '';
            (window as any).__openclawLastMotionPreset =
              nextMotion.includes('"index":1')
                ? 'gentle-nod'
                : nextMotion.includes('"index":5')
                  ? 'wave-soft'
                  : nextMotion.includes('"index":3')
                    ? 'wake-soft'
                    : nextMotion.includes('"index":2')
                      ? 'guarded-soft'
                      : nextMotion.includes('"index":4')
                        ? 'slow-blink'
                        : nextMotion.includes('"index":0')
                          ? 'lean-in-soft'
                          : nextExpression === 'warm'
                            ? 'gentle-nod'
                            : nextExpression === 'curious'
                              ? 'lean-in-soft'
                              : nextExpression === 'concerned'
                                ? 'guarded-soft'
                                : 'still-attentive';
            (window as any).__openclawBridgeMotionKey = `${Date.now()}-${Math.random()}`;
            window.dispatchEvent(new CustomEvent('openclaw-companion-action', {
              detail: {
                preset: (window as any).__openclawLastMotionPreset,
                token: (window as any).__openclawBridgeMotionKey,
