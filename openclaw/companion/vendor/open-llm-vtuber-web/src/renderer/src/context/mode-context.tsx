...
export const ModeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const startupMode = window.api?.getCurrentMode?.();
    if (!startupMode) return;

    setModeState(startupMode);

    // If the main process already considers the app to be in pet mode,
    // explicitly finish the startup handshake after the renderer mounts.
    // This avoids losing the pre-mode-changed event during initial load.
    if (startupMode === 'pet') {
