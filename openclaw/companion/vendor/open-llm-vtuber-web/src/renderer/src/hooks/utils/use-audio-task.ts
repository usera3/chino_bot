...
export const useAudioTask = () => {
  ...
  const handleAudioPlayback = (options: AudioTaskOptions): Promise<void> => new Promise((resolve) => {
    ...
    if (lappAdapter && motions?.length) {
      const model = lappAdapter.getModel?.();
      if (model) {
        for (const motion of motions) {
          try {
            const group = motion.group ?? "";
            const priority = motion.priority ?? 3;
            if (motion.random) {
              model.startRandomMotion(group, priority);
            } else {
              model.startMotion(group, motion.index ?? 0, priority);
            }
            break;
          } catch (error) {
            console.error("Failed to play motion action:", error);
          }
        }
      }
    }
