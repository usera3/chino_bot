...
export const applyPosition = (
  ...
  const initXshiftPixels = Number(initialXshift || 0) * dpr;
  const initYshiftPixels = Number(initialYshift || 0) * dpr;
  const x = canvasWidth / 2 + initXshiftPixels;
  const y = canvasHeight / 2 + initYshiftPixels;
...
export const useLive2DResize = ({
  ...
  useEffect(() => {
    ...
    const retryTimers = [300, 900, 1800].map((delay) => window.setTimeout(() => {
      handleResize();
    }, delay));
