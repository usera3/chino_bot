...
const BRIDGE_EVENT_ENDPOINT = 'http://127.0.0.1:18812/event';
...
export const useLive2DModel = ({
  ...
  const applyInitialDesktopPetPosition = useCallback(() => {
    ...
    const x = width / 2 + Number(modelInfo?.initialXshift || 0) * dpr;
    const y = height / 2 + Number(modelInfo?.initialYshift || 0) * dpr;
    ...
    try {
      ...
      console.debug('Failed to apply initial desktop pet position:', error);
    }
  }, [modelInfo?.initialXshift, modelInfo?.initialYshift]);

  const sendBridgeEvent = useCallback(async (
    type: 'click' | 'drag',
    note: string,
  ) => {
    try {
      await fetch(BRIDGE_EVENT_ENDPOINT, {
        method: 'POST',
        headers: {
      ...
      console.debug(`[Bridge] Failed to send ${type} event:`, error);
    }
  }, []);
      ...
      try {
        ...
        if (baseUrl && modelDir) {
          ...
          setTimeout(() => {
            ...
            initializeLive2D();
            window.setTimeout(() => {
              applyInitialDesktopPetPosition();
            }, 1000);
            window.setTimeout(() => {
              applyInitialDesktopPetPosition();
            }, 1800);
  ...
  }, [modelInfo?.url, modelInfo?.kScale, applyInitialDesktopPetPosition]);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (isDragging) {
      ...
      void sendBridgeEvent('drag', 'Model dragged on desktop');
      ...
      if (timeElapsed < TAP_DURATION_THRESHOLD_MS && distanceMoved < DRAG_DISTANCE_THRESHOLD_PX) {
        ...
        void sendBridgeEvent('click', 'Model clicked on desktop');
      }
  ...
  }, [isDragging, canvasRef, modelInfo, sendBridgeEvent]);
