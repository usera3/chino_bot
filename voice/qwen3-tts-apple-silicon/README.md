...
This repo includes the voice reference assets and the small config files needed
for the current `akari2` setup. The two large `.safetensors` weight files are
not committed in Git; restore them with `./download-model.sh`.
